...
export const dsl = defineDsl(analyzer);
...
export const renderFrom = {
  ...
  /**
   * Generate from a string in an in-memory filesystem — the testing front
   * door. Only exported aliases are emitted; an unexported alias is a helper
   * (e.g. a `type Body = [...]` shared across several themed diagrams).
   */
  code: (code: string, fileName = "diagram.ts") =>
    dsl.code(code, fileName).map(emit),
  source: (source: SourceFile) => dsl.source(source).map(emit),
};
